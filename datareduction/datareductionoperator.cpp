...
namespace DRO {
   ...
   bool VariablePTensorDiagonal::reduceData(const SpatialCell* cell,char* buffer) {
      ...
      # pragma omp parallel
      {
         ...
         for (vmesh::LocalID n=0; n<cell->get_number_of_velocity_blocks(popID); n++) {
            Realf temp[WID3];
            const Realf* block_data = cell->get_data(n, popID, temp);
   ...
   bool MaxDistributionFunction::reduceDiagnostic(const SpatialCell* cell,Real* buffer) {
      ...
      #pragma omp parallel 
      {
	    ...
	    for (uint k = 0; k < WID; ++k) for (uint j = 0; j < WID; ++j) for (uint i = 0; i < WID; ++i) {
	       threadMax = max((Real)(block_data[cellIndex(i,j,k)]), threadMax);
            }
         }
   ...
   bool MinDistributionFunction::reduceDiagnostic(const SpatialCell* cell,Real* buffer) {
      ...
      #pragma omp parallel 
      {
	    ...
	    for (uint k = 0; k < WID; ++k) for (uint j = 0; j < WID; ++j) for (uint i = 0; i < WID; ++i) {
	       threadMin = min((Real)(block_data[cellIndex(i,j,k)]), threadMin);
            }
         }
         ...
         for (vmesh::LocalID n=0; n<cell->get_number_of_velocity_blocks(popID); ++n) {
            ...
            * parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVZ];
            ...
            for( vector< array<uint, 3> >::const_iterator it = vCellIndices.begin(); it != vCellIndices.end(); ++it ) {
               ...
               thread_n_sum += block_data[cellIndex(i,j,k)]*DV3;
            }
   ...
   bool VariablePrecipitationDiffFlux::reduceData(const SpatialCell* cell,char* buffer) {
      ...
      # pragma omp parallel
      {
		 ...
		 * parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVZ];
	       ...
	       thread_lossCone_sum[binNumber] += block_data[cellIndex(i,j,k)] * countAndGate * normV*normV * DV3;
      ...
      # pragma omp parallel
      {
	    ...
	    for (uint k = 0; k < WID; ++k) for (uint j = 0; j < WID; ++j) for (uint i = 0; i < WID; ++i) {
		 ...
		 + (k + HALF)*parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVZ];
	       ...
	       thread_E0_sum += block_data[cellIndex(i,j,k)] * ENERGY * DV3;
	       if (ENERGY > E1limit) thread_E1_sum += block_data[cellIndex(i,j,k)] * ENERGY * DV3;
	       if (ENERGY > E2limit) thread_E2_sum += block_data[cellIndex(i,j,k)] * ENERGY * DV3;
            }
         }
