...
inline void CompressedBlock::set(float* array) {
    ...
    //std::cerr << "R = " << range << " = " << (void*)(max.i - min.i + 0x1FFFFF + 0x1FFFF) << ", M = " << (void*)magic << std::endl;
    ...
    if (n_values >= BLOCK_SIZE - 4)
    {
        ...
        for (int i = 0; i < BLOCK_SIZE / VECL; i++) 
        {
            values.load(array + i*VECL);
            values /= range;
            values = magic - values;
            values >>= 5;
            values.store(p);
